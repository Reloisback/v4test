...
class Changes(commands.Cog):
    ...
    async def get_admin_info(self, user_id: int):
        self.c_settings.execute("""
            SELECT a.id, a.is_initial, a.level, GROUP_CONCAT(aa.alliance_id) as alliances
            FROM admin a
            LEFT JOIN admin_alliances aa ON a.id = aa.admin_id
            WHERE a.id = ?
            GROUP BY a.id
        """, (user_id,))
    ...
    async def get_admin_alliances(self, user_id: int, guild_id: int):
        try:
            ...
            admin_result = self.c_settings.fetchone()
            ...
            self.c_settings.execute("""
                ...
                JOIN alliance_list al ON aa.alliance_id = al.alliance_id
                WHERE aa.admin_id = ?
            """, (user_id,))
            special_alliances = self.c_settings.fetchall()
            ...
            with sqlite3.connect('db/alliance.sqlite') as alliance_db:
                ...
                cursor.execute("""
                    ...
                    JOIN guild_alliances ga ON al.alliance_id = ga.alliance_id
                    WHERE ga.guild_id = ?
                    ORDER BY al.name
                """, (guild_id,))
            ...
            all_alliances.sort(key=lambda x: x[1])
...
class HistoryView(discord.ui.View):
    ...
    async def furnace_changes_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            select_embed = discord.Embed(
                title="🔥 Alliance Selection - Furnace Changes",
                description=(
                    "Please select an alliance to view furnace changes:\n\n"
                    "**Permission Details**\n"
                    "━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    async def nickname_changes_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            select_embed = discord.Embed(
                ...
                description=(
                    "Please select an alliance to view nickname changes:\n\n"
                    "**Permission Details**\n"
                    "━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"👤 **Access Level:** `{'Global Admin' if is_initial == 1 else 'Server Admin'}`\n"
                    f"🔍 **Access Type:** `{'All Alliances' if is_initial == 1 else 'Server + Special Access'}`\n"
    ...
    async def show_main_menu(self, interaction: discord.Interaction):
        try:
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    "Ana menüye dönülürken bir hata oluştu.", 
                    ephemeral=True
                )
            else:
                await interaction.followup.send(
